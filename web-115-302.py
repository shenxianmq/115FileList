...
try:
    ...
    run(
        [executable, "-m", "pip", "install", "-U", "flask", "posixpatht", "python-115"],
        check=True,
    )
    from flask import Flask, request, redirect, render_template_string, Response
    from p115 import P115FileSystem
...
def query(path: str):
    ...
    fid = attr["id"]
    if fid == 0:
        header = f'<strong><a href="{origin}?id=0&method=list" style="border: 1px solid black; text-decoration: none">/115</a></strong>'
    else:
        ancestors = fs.get_ancestors(int(attr["id"]))
        info = ancestors[-1]
        header = (
            f'<strong><a href="{origin}?id=0" style="border: 1px solid black; text-decoration: none">/115</a></strong>'
            + "".join(
